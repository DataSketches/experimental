...
public interface MemoryRequest {
   ...
   */
  MemoryImpl request(long capacityBytes);

  /**
   * Request for allocate and copy.
   *
   * <p>Request to allocate new Memory with the capacityBytes; copy the contents of origMem
   * from zero to copyToBytes.</p>
   *
   * @param origMem The original Memory, a portion, starting at zero, which will be copied
   * to the newly allocated Memory. This reference must not be null.
   ...
   * allocated Memory. The upper region of the new Memory may or may not be cleared
   * depending on the implementation.
   *
   * @param capacityBytes the desired new capacity of the newly allocated Memory in bytes.
   * @return The new Memory with the given capacity. If this request is refused it will be
   * null.
   */
  MemoryImpl request(MemoryImpl origMem, long copyToBytes, long capacityBytes);

  /**
   * The given Memory with its capacity is to be freed. It is assumed that the
   * implementation of this interface knows the type of Memory that was created and how
   * to free it.
   * @param mem The Memory to be freed
   */
  void free(MemoryImpl mem);
   ...
   * newMem, if desired.
   */
  void free(MemoryImpl memToFree, MemoryImpl newMem);
}
