...
public class CountMin{
  ...
  /**
   * Constructs and initializes a CountMin sketch.
  ...
  /**
   * Determine an estimate for the frequency of key, specified as a long.
   * It is guaranteed that with probability at least 1-delta
   * 1) getEstimate(key) >= real count
   * 2) getEstimate(key) <= real count + getMaxError() 
   * 
   ...
   * Determine an estimate for the frequency of key, specified as a long.
   * It is guaranteed that with probability at least 1-delta
   * 1) getEstimateUpperBound(key) >= real count
   * 2) getEstimateUpperBound(key) <= real count + getMaxError() 
   * 
   * @param key whose count estimate is returned.
   * @return the approximate count for the key.
   */
   ...
   * Determine an estimate for the frequency of key, specified as a long.
   * It is guaranteed that with probability at least 1-delta
   * 1) getEstimateLowerBound(key) >= real count - getMaxError()
   * 2) getEstimateLowerBound(key) >= real count
   * 
   * @param key whose count estimate is returned.
  ...
  /**
   * Returns a bound on the error of any estimate returned by the sketch (the bound holds
   * for each estimate with probability at least 1-delta).
   *  
   * Note that the error is one sided. If the real count is realCount(key) then
   * getEstimate(key) >= realCount(key). The guarantee of the sketch is that, for any fixed key, 
   ...
   * 
   * @param other Another CountMin sketch. Must have been created using the same hash functions (i.e., same seed to MurmurHash)
