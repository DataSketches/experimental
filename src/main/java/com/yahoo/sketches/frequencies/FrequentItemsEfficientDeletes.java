 ...
 * @author Justin8712
 */
public class FrequentItemsEfficientDeletes extends FrequencyEstimator{
  ...
  public FrequentItemsEfficientDeletes(double errorParameter) {
    ...
    counters = new HashMapWithEfficientDeletes(4*this.maxSize/3);
    this.offset = 0;
    if (this.maxSize < 100) 
  ...
  private void purge()
  {
   ...
   /**
    * @param other
    * Another FrequentItemsEfficientDeletes sketch. Potentially of different size. 
   ...
   public FrequencyEstimator merge(FrequencyEstimator other) {
     if (!(other instanceof FrequentItemsEfficientDeletes)) throw new IllegalArgumentException("SpaceSaving can only merge with other SpaceSaving");
       FrequentItemsEfficientDeletes otherCasted = (FrequentItemsEfficientDeletes) other;
