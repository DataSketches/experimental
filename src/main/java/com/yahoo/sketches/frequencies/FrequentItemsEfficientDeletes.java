 ...
 * keep roughly 1/eps counters. When the sketch is updated with a key and increment, the
 * corresponding counter is incremented or, if there is no counter for that key, a new counter is
 * created. If the sketch reaches its maximal allowed size, it removes some counter and decrements
 * others. The logic of the frequent-items sketch is such that the stored counts and real counts are
 * never too different. More specifically, for any key k, the sketch can return an estimate of the
 * true frequency of k, along with upper and lower bounds on the frequency (that hold
 * deterministically). The sketch is guaranteed that, with high probability over the randomness of
 ...
 *
 * <p>Uses HashMapWithEfficientDeletes
 *
 * @author Justin8712
 */
public class FrequentItemsEfficientDeletes {

  private HashMapWithEfficientDeletes counters;
  ...
  // sample_Size is maximum number of samples used to compute approximate median of counters when
  // doing decrement
  private int sample_size;
  private long[] samples;

  /**
   * @param errorParameter Determines the accuracy of the estimates returned by the sketch. The
   ...
   */
  public FrequentItemsEfficientDeletes(final double errorParameter) {
    if (errorParameter <= 0) {
      throw new IllegalArgumentException("Received negative or zero value for maxSize.");
    }
    this.maxSize = (int) (1 / errorParameter) + 1;
    counters = new HashMapWithEfficientDeletes(this.maxSize);
    this.offset = 0;
    if (this.maxSize < 100) {
   ...
   *         changed.
   */
  public FrequentItemsEfficientDeletes merge(final FrequentItemsEfficientDeletes other) {
