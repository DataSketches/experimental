 ...
 * @author Justin8712
 */
public class FrequentItemsLPWR extends FrequencyEstimator{
  ...
  public FrequentItemsLPWR(double errorParameter) {
    ...
    counters = new HashMapLinearProbingWithRebuilds(4*this.maxSize/3);
    this.offset = 0;
    if (this.maxSize < 100) 
  ...
  private void purge()
  {
   ...
   /**
    * @param other
    * Another FrequentItemsLPWR sketch. Potentially of different size. 
   ...
   public FrequencyEstimator merge(FrequencyEstimator other) {
     if (!(other instanceof FrequentItemsLPWR)) throw new IllegalArgumentException("SpaceSaving can only merge with other SpaceSaving");
       FrequentItemsLPWR otherCasted = (FrequentItemsLPWR) other;
