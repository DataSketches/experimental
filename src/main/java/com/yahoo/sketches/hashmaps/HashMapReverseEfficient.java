...
/**
 * Implements a linear-probing based hash table. Supports
 * a purge operation that removes all keys in the table whose
 * associated values are below a threshold. This purge operation 
 * is done starting at the ``back'' of the array and moving toward
 * the front.
 */
public class HashMapReverseEfficient extends HashMap {
  ...
  /**
   * Turns the HashMapReverseEfficient object into a string listing
   * properties of the table and all the (key, value) pairs that the table contains.
  ...
  /**
   * Turns a string specifying a HashMapReverseEfficient object 
   * into a HashMapReverseEfficient object.
  ...
  public static HashMapReverseEfficient StringToHashMapReverseEfficient(String string) {
    ...
    if(tokens.length < 2) {
      throw new IllegalArgumentException("Tried to make HashMapReviseEfficient out of string not long enough to specify length and capacity.");
    }
  ...
  /**
   * Turns an array of strings specifying a HashMapReverseEfficient object 
   * into a HashMapReverseEfficient object.
   ...
   */
  public static HashMapReverseEfficient StringArrayToHashMapReverseEfficient(String[] tokens, int ignore) {
    if(ignore < 0) {
      throw new IllegalArgumentException("ignore parameter is negative in StringArrayToHashMapReverseEfficient.");
    }
    if(tokens.length < 2) {
      throw new IllegalArgumentException("Tried to make HashMapReviseEfficient out of string not long enough to specify length and capacity.");
    }
