...
class Auxiliary { /* auxiliary data structure for answering quantile queries */
  ...
  private static int countSamplesInSketch (int mqK, long mqN, double [][] mqLevels, 
    ...
    for (int lvl = 0; lvl < mqLevels.length; lvl++) {
      if (mqLevels[lvl] != null) { count += mqK; }
    }
    return count;
  }
  ...
  private static void populateAuxiliaryArrays (int mqK, long mqN, double [][] mqLevels, 
    ...
    /* The specific sorting / merging scheme that will applied
       to the auxiliary arrays requires that the contents of the
       "levels" precede the contents of the "base buffer". That way 
       each successive block of length = k will already be sorted */
  ...
  static Auxiliary constructAuxiliary (int mqK, long mqN, double [][] mqLevels, 
    ...
    populateAuxiliaryArrays (mqK, mqN, mqLevels, mqBaseBuffer, mqBaseBufferCount, items, accum, numSamples);
  ...
  */
  ...
  private static int searchForChunkContainingPos (long [] arr, long q, int l, int r) {
    ...
    if (l+1 == r) {return l;}
      ...
      int m = l + (r - l) / 2;
      if (arr[m] <= q) { return(searchForChunkContainingPos(arr, q, m, r)); }
      else             { return(searchForChunkContainingPos(arr, q, l, m)); }
    }
  }
